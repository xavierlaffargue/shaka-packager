...
void BuildStreamInfTag(const MediaPlaylist& playlist,
                       ...
                       bool detect_cea, // Added detect_cea
                       std::string* out) {
  DCHECK(out);
  ...
  // This behavior is modified if detect_cea is true.
  if (playlist.stream_type() == MediaPlaylist::MediaPlaylistStreamType::kVideo ||
      playlist.stream_type() == MediaPlaylist::MediaPlaylistStreamType::kVideoIFramesOnly) {
    bool cea_found_for_this_stream = false;
    if (detect_cea) {
      const auto& video_info = playlist.GetMediaInfo().video_info();
      if (video_info.has_cea608_caption_info()) {
        const auto& cea_info = video_info.cea608_caption_info();
        if (cea_info.has_cc1() || cea_info.has_cc2() || cea_info.has_cc3() ||
            cea_info.has_cc4()) {
          cea_found_for_this_stream = true;
        }
      }
    }

    if (cea_found_for_this_stream) {
      // If CEA captions are detected for this stream and detect_cea is on,
      // link to the "CC" group. The actual EXT-X-MEDIA tags for "CC"
      // will be generated by BuildCea608MediaTags.
      tag.AddQuotedString("CLOSED-CAPTIONS", "CC");
    } else {
      // No CEA captions detected for this stream, or detect_cea is off.
      tag.AddString("CLOSED-CAPTIONS", "NONE");
    }
  }
  // Note: Audio streams do not have CLOSED-CAPTIONS in EXT-X-STREAM-INF.
...
void AppendPlaylists(const std::string& default_audio_language,
                     ...
                     bool detect_cea, // Added detect_cea flag
                     const std::string& base_url,
  ...
  // Helper function to generate EXT-X-MEDIA tags for CEA-608 captions.
  // This is defined here to be within the anonymous namespace or as a static helper if preferred.
  // For the purpose of this diff, defining it before its first use in AppendPlaylists.
  auto build_cea608_media_tags = 
    [](const std::list<const MediaPlaylist*>& all_video_type_playlists,
       const std::string& default_text_language_for_cea) -> std::string {
    std::string cea_tags_content;
    std::set<std::string> generated_instream_ids;
    bool first_cea_tag_overall = true;

    for (const auto* video_playlist : all_video_type_playlists) {
      if (video_playlist->stream_type() != MediaPlaylist::MediaPlaylistStreamType::kVideo &&
          video_playlist->stream_type() != MediaPlaylist::MediaPlaylistStreamType::kVideoIFramesOnly)
        continue;

      const auto& video_info = video_playlist->GetMediaInfo().video_info();
      if (video_info.has_cea608_caption_info()) {
        const auto& cea_info = video_info.cea608_caption_info();
        struct CeaChannelInfo {
          bool present;
          const char* id_name;
        };
        CeaChannelInfo channels[] = {
            {cea_info.has_cc1(), "CC1"},
            {cea_info.has_cc2(), "CC2"},
            {cea_info.has_cc3(), "CC3"},
            {cea_info.has_cc4(), "CC4"},
        };

        for (const auto& channel : channels) {
          if (channel.present) {
            if (generated_instream_ids.find(channel.id_name) == generated_instream_ids.end()) {
              Tag tag("#EXT-X-MEDIA", &cea_tags_content);
              tag.AddString("TYPE", "CLOSED-CAPTIONS");
              tag.AddQuotedString("GROUP-ID", "CC");
              tag.AddQuotedString("NAME", channel.id_name);
              tag.AddQuotedString("LANGUAGE", default_text_language_for_cea.empty() ? "und" : default_text_language_for_cea);
              tag.AddQuotedString("INSTREAM-ID", channel.id_name);

              bool is_default_for_group = (std::string(channel.id_name) == "CC1" && first_cea_tag_overall);
              tag.AddString("DEFAULT", is_default_for_group ? "YES" : "NO");
              tag.AddString("AUTOSELECT", is_default_for_group ? "YES" : "NO");
              cea_tags_content.append("\n");
              generated_instream_ids.insert(channel.id_name);
              if (is_default_for_group) {
                  first_cea_tag_overall = false;
              }
            }
          }
        }
    ...
    return cea_tags_content;
  };

  if (detect_cea && (!video_playlists.empty() || !iframe_playlists.empty())) {
    std::list<const MediaPlaylist*> all_video_type_playlists = video_playlists;
    all_video_type_playlists.insert(all_video_type_playlists.end(), 
                                    iframe_playlists.begin(), iframe_playlists.end());
    std::string cea_tags = build_cea608_media_tags(all_video_type_playlists, default_text_language);
    if (!cea_tags.empty()) {
      content->append("\n");
      content->append(cea_tags);
    }
  }
  ...
  for (const auto& variant : variants) {
    ...
    for (const auto& playlist : video_playlists) {
      BuildStreamInfTag(*playlist, variant, base_url, detect_cea, content);
    }
  }
...
bool MasterPlaylist::WriteMasterPlaylist(
  ...
  AppendPlaylists(default_audio_language_, default_text_language_, detect_cea_,
                  base_url, playlists, &content);
